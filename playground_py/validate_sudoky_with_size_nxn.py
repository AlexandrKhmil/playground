class Sudoku(object):
    ...
    def done_or_not(self):
        check = lambda x : set(x) == set(range(1, len(self.data) + 1))
        # Check row
        for l in self.data:
            if not check(l):
                print('1')
        ...
        for i in range(0, len(self.data)): 
            if not check([item[i] for item in self.data]):
                print('2')
                return False
        # Check cells
        for i in range(0, len(self.data), int(len(self.data) ** 0.5)):
            for j in range(0, len(self.data), int(len(self.data) ** 0.5)): 
                if not check([item for sublist in list(map(lambda x: x[j:j + int(len(self.data) ** 0.5)], self.data[i:i+int(len(self.data) ** 0.5)])) for item in sublist]):
                    print('3')
                    return False
        return True

    def is_valid(self):
        if not self.valid: return self.valid 
